...
def simulated_annealing(orders, machines, initial_temp=1000, cooling_rate=0.95, max_iterations=1000):
    # Generate an initial schedule from the existing order list
    current_orders = orders.copy()
    current_schedule = schedule_orders(current_orders, machines)
    current_penalty = calculate_penalty(current_orders, current_schedule)
    ...
    for iteration in range(max_iterations):
        ...
        # Track the best swap found in this iteration
        for i in range(len(current_orders)):
            for j in range(i + 1, len(current_orders)):
                # Create a swap
                current_orders[i], current_orders[j] = current_orders[j], current_orders[i]

                # Calculate the new schedule and penalty
                new_schedule = schedule_orders(current_orders, machines)
                new_penalty = calculate_penalty(current_orders, new_schedule)

                # Calculate the change in penalty
                penalty_change = new_penalty - current_penalty

                # Decide whether to accept the new solution
                if penalty_change < 0:  # If new penalty is better, accept
                    current_penalty = new_penalty
                    current_schedule = new_schedule
                    improved = True
                else:
                    # Accept with a probability based on temperature
                    acceptance_probability = np.exp(-penalty_change / temperature)
        ...
        # Update the best found penalty if necessary
        if current_penalty < best_penalty:
            best_penalty = current_penalty
            best_schedule = current_schedule

        # Early stopping if the temperature is very low
        if temperature < 1e-3:
            break

# Run the Simulated Annealing optimization
optimized_schedule_sa, optimized_penalty_sa = simulated_annealing(orders, machines)

# Visualize and print the optimized schedule from Simulated Annealing
machine_schedules_sa = convert_sched_O_to_sched_M(optimized_schedule_sa)
draw_schedule(machine_schedules_sa)

# Print the penalty for the optimized schedule found by Simulated Annealing
print(f"Optimized penalty (Simulated Annealing): {optimized_penalty_sa}")
